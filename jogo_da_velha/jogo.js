...
$('#btn_iniciar_jogo').click(function(){
	...
	//valida a digitação dos appelidos dos jogadores
	if($("#entrada_apelido_jogador_1").val() == ''){
		...
		alert('Apelido do jogador 2 não foi prenchido');
		return false;
	}
...
$('.jogada').click( function(){
 ...
 var id_campo_clicdo = this.id;
 $('#'+id_campo_clicdo).off();
 jogada(id_campo_clicdo); 
...
function ganhador(pontos){
	if (pontos == -3) {
		...
		alert(jogada_1 + ' Venceu a partida !!!!!!');
		$('.jogada').off();

	}else if(pontos == 3){
		var jogada_2 = $('#entrada_apelido_jogador_2').val();
		alert(jogada_2 + ' Venceu a partida !!!!!!');
		$('.jogada').off();
	}
}
